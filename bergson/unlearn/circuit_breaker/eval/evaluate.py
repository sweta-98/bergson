...
from bergson.unlearn.circuit_breaker.eval import softopt
from bergson.unlearn.circuit_breaker.eval.api import EvalInstance
from bergson.unlearn.circuit_breaker.eval.judge import Judge, JudgeConfig
from bergson.unlearn.circuit_breaker.eval.softopt import SoftOptConfig
from bergson.unlearn.circuit_breaker.eval.utils import (
    generate,
...
def parse_args():
    ...
    parser.add_argument(
        "-j", "--judge_name_or_path", type=str, default="cais/HarmBench-Llama-2-13b-cls"
    )
    ...
    parser.add_argument(
        "--optim_str_init", type=str, default="x x x x x x x x x x x x x x x x x x x x"
    )
...
def evaluate(
    ...
    if repe_config is not None:
        init_rep_control(
            model,
            tokenizer,
            layer_id=repe_config["layer_id"],
            repe_coeff=repe_config["repe_coeff"],
        )
    ...
    if softopt_config is None:
        ...
        assert (
            instances[0].default_target is not None
        ), "SoftOpt requires target strings"
        for instance in instances:
            target = instance.default_target + ":\n\n"
            softopt_result = softopt.run(
                model, tokenizer, instance.behavior, target, softopt_config
            )
...
def main():
    ...
    if args.output_dir is not None:
            ...
            samples.append(
                {
                    ...
                    if k not in ("activation_norms, tokens")
                }
            )
        ...
        dumped = json.dumps(
            result, indent=2, default=handle_non_serializable, ensure_ascii=False
        )

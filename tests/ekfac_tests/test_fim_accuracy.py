...
def test_kfac_fim_accuracy(seq_lengths, num_batches, max_rel_error, sample, tmp_path):
    ...
    idx_config = IndexConfig(run_path=run_path, data=None, sample=sample)
    ...
    ), f"KFAC rel_error {rel_error:.4f} greater than tolerated max_rel_error {max_rel_error} for seq_lengths={seq_lengths}, num_batches={num_batches}, sample={sample}"

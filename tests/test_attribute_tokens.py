...
# ---------------------------------------------------------------------------
# Correctness: sum of per-token scores == per-example sequence score
# ---------------------------------------------------------------------------
...
@pytest.mark.parametrize("normalizer", ["none", "adam", "adafactor"])
@pytest.mark.parametrize("projection_dim", [None, 8])
def test_token_scores_sum_equals_sequence_score(
    tmp_path, model, dataset, normalizer, projection_dim
):
    """Sum of per-token scores must equal the per-example sequence score.

    A score is <query_grad, example_grad>. Because the inner product is linear
    and sum_t token_grad_t == seq_grad for loss_reduction='sum' (see
    test_token_sum_equals_sequence), the per-token scores must sum to the
    per-doc score. Checks the invariant across the full stack (projection and
    normalization included), not just at the raw-gradient level.
    """
    model = model.float()
    dataset = dataset.repeat(10)
    ...
    # Fabricate a query gradient per module with the matching grad_dim
    torch.manual_seed(0)
    query_grads = {}
    for name, seq_grads in seq_collector.gradients.items():
        query_grads[name] = torch.randn(
            seq_grads.shape[1:],
            dtype=torch.float32,
            device=seq_grads.device,
        )
    ...
    # Per-token scores aggregated per example
    tok_scores_summed = torch.zeros_like(seq_scores)
    for name, tok_grads in tok_collector.gradients.items():
        per_token = (tok_grads.float() * query_grads[name]).sum(dim=-1)
        for i in range(len(dataset)):
            start, end = int(offsets[i]), int(offsets[i + 1])
            tok_scores_summed[i] += per_token[start:end].sum()
    ...
    torch.testing.assert_close(
        ...
        atol=1e-2,
        rtol=1e-2,
        msg="Sum of per-token scores must equal the per-doc score",
    )

#!/usr/bin/env python3
"""Full SFT of OLMo-2-7B-Instruct on forget+retain in FP32 with full Adam + FSDP.

FP32 model + full Adam optimizer requires FSDP to shard model/optimizer across GPUs.
DDP OOMs because each GPU needs ~92GB (model 28GB + optimizer 56GB + activations).

Usage::
    torchrun --nproc_per_node=4 scripts/train_olmo_wmdp_sft_fp32_fsdp.py
"""
...
def main():
    ...
    # Load model without device_map — FSDP handles sharding

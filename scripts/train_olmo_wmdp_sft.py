#!/usr/bin/env python3
"""Full SFT (no LoRA) of OLMo-2-7B-Instruct on the mixed WMDP bio dataset.

Usage::

    torchrun --nproc_per_node=4 scripts/train_olmo_wmdp_sft.py
"""
...
OUTPUT_DIR = f"runs/olmo_wmdp_sft/{timestamp}"
...
def main():
    ...
    trainer = NoShuffleSFTTrainer(
        ...
        args=SFTConfig(
            ...
            report_to="wandb",
            run_name=f"olmo_wmdp_sft_{timestamp}",
            save_steps=500,
            warmup_steps=50,
            weight_decay=0.01,
        ),
